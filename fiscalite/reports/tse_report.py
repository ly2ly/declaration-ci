...
class declaration_tse(osv.osv):
    ...
    _inherit="declaration.tse"

    def tse_report_xls(self, cr, uid, ids, context=None): # fonction report tse
        fl = StringIO()
        if context is None :
                context={}
        wbk=xlwt.Workbook()
        ...
        data['form'] = res
        try:
                form_id= self.pool.get('ir.model.data').get_object_reference(cr, uid, 'tse_report_xls','tse_form')[1]
        except ValueError:
                form_id=False
        return{
