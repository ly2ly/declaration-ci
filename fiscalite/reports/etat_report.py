...
class declaration_etat(osv.osv):
    ...
    _inherit="declaration.etat"

    def etat_report_xls(self, cr, uid, ids, context=None): # fonction report etat
        fl = StringIO()
        if context is None :
                context={}
        wbk=xlwt.Workbook()
        ...
        data['form'] = res
        try:
                form_id= self.pool.get('ir.model.data').get_object_reference(cr, uid, 'etat_report_xls','etat_form')[1]
        except ValueError:
                form_id=False
        return{
