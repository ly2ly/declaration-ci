...
class declaration_its(osv.osv):
    ...
    _inherit="declaration.its"

    def its_report_xls(self, cr, uid, ids, context=None): # fonction report its
        fl = StringIO()
        if context is None :
                context={}
        wbk=xlwt.Workbook()
        ...
        data['form'] = res
        try:
                form_id= self.pool.get('ir.model.data').get_object_reference(cr, uid, 'its_report_xls','its_form')[1]
        except ValueError:
                form_id=False
        return{
