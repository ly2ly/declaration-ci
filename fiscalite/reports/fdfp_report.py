...
class declaration_fdfp(osv.osv):
    ...
    _inherit="declaration.fdfp"

    def fdfp_report_xls(self, cr, uid, ids, context=None): # fonction report fdfp
        fl = StringIO()
        if context is None :
                context={}
        wbk=xlwt.Workbook()
        ...
        data['form'] = res
        try:
                form_id= self.pool.get('ir.model.data').get_object_reference(cr, uid, 'fdfp_report_xls','fdfp_form')[1]
        except ValueError:
                form_id=False
        return{
